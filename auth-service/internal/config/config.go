...
type HTTPServer struct {
	Address     string        `yaml:"address" env-default:":8080" env:"HTTP_SERVER_ADDRESS"`
...
func MustLoad() *Config {
	...
	if cfg.AllowLocalhostOrigin != "" {
		for _, localhostOrigin := range strings.Split(cfg.AllowLocalhostOrigin, ",") {
			localhostOrigin = strings.TrimSpace(localhostOrigin)
			if localhostOrigin == "" {
				continue
			}

			origins = append(origins, localhostOrigin)
		}
	}
